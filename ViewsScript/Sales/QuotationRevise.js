$(window).load(function () {
    ...
    var ReviseTermsCondition = $('#ReviseTermsCondition').val();
        ...
        success: function (data) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    cQCPerson.select2("val", ReviseContactPersonID);
                },
                error: function () {
                }
            });
    ...
    $('#termsCondition').val(ReviseTermsCondition);
...
$('#packQuantity').on('input', function () {
    ...
    if ($("#packID").val() == "") {
        toastr["error"]("Please select package first");
        return false;
    }
...
var defineGridProject = function (data) {
    ...
    grid.jqGrid({
        datatype: 'local',
        data: data,
        colNames: ["", "", "Package Name", "Quantity", "Unit Price (RM)", "GST (RM)", "Price (RM)", "Remarks", "Edit", "Delete"],
        colModel: [
        ...
        { name: 'GSTPrice', index: 'GSTPrice', align: 'right', width: 110 },
...
$('#btnReviseQuotation').click(function (e) {
    ...
    var r = confirm("Are you sure REVISE the quotation?");
    if (r == true) {
        ...
        $.ajax({
            url: '/NewQuotation/saveReviseQuotation',
            type: 'POST',
            contentType: 'application/json; charset=utf-8',
            ...
            success: function (data) {
                if (data.Result == "OK") {
                    $.LoadingOverlay("hide");
                    bootbox.dialog({
                        message: "Revise Quotation Successfully Saved",
